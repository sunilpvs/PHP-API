<?php
// endpoint for rejecting a modification request. This will be called by the approver for modification requests only.
// /api/lms/modification/reject.php
...
try {
    $logger->log("Received $method request at /api/lms/modification/reject.php");
    ...
    $result = $leaveReqOb->rejectModificationRequest(
        $id,
        $adminComments,
        $updatedBy,
        $module,
    ...
    if ($result) {
        ...
        echo json_encode(['message' => 'Modification request rejected successfully.']);
    } else {
        http_response_code(400);
        echo json_encode(['error' => 'Failed to reject modification request. Please ensure it is a pending modification request.']);
    }
} catch (Exception $e) {
