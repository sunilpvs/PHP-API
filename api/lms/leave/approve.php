<?php
// endpoint for approving a leave request. This will be called by the approver for new leave requests only.
// /api/lms/leave/approve.php
...
try {
    $logger->log("Received $method request at /api/lms/leave/approve.php");
    ...
    $result = $leaveReqOb->approveLeaveRequest(
        $id,
        $adminComments,
        $updatedBy,
        $module,
    ...
    if ($result) {
        ...
        echo json_encode(['message' => 'Leave request approved successfully.']);
    } else {
        http_response_code(400);
        echo json_encode(['error' => 'Failed to approve leave request. Please ensure it is a pending new leave request.']);
    }
} catch (Exception $e) {
