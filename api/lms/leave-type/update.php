<?php 
// endpoint for updating an existing leave type.
// /api/lms/leave-type/update.php
// only for updating leave types
...
try {
    $logger->log("Received $method request at /api/lms/leave-type/update.php");

    // TODO: only allow users with HR_ADMIN role to update leave types.
    
    // get the leave type ID from the query parameters
    if (!isset($_GET['id'])) {
    ...
    // check the existence of the leave type
    ...
    $leaveTypeOb->updateLeaveType($id, $leaveTypeName, $description, $username, $module, $username);

    http_response_code(200); // OK
    echo json_encode(['message' => 'Leave type updated successfully.']);

} catch (Exception $e) {
    $logger->log("Error in /api/lms/leave-type/update.php: " . $e->getMessage());
    http_response_code(500); // Internal Server Error
    echo json_encode(['error' => 'An error occurred while updating the leave type.']);  
}
